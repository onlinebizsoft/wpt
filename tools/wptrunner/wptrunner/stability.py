...
def write_taskcluster_summary_inconsistent(log, inconsistent, iterations):
    ...
    log("\n")
    log("These may be pre-existing or new flakes. Please try to reproduce via "
        "the above WPT command and determine if your change introduced the "
        "flake. If you are unable to reproduce the problem, please tag "
...
def write_taskcluster_summary_slow_tests(log, slow):
    ...
    log("\n")
    log("These may be pre-existing or new slow tests. Slow tests indicate that "
        "a test ran very close to the test timeout limit and so may become "
        "TIMEOUT-flaky in the future. Consider speeding up the test or "
